...
def parse_geomagnetic_data(filepath):
    ...
    return data
def test_CM6_above_SQfield():
    data = parse_geomagnetic_data("tests/test_values_CM6_v2_aboveSQfield_20251015.txt")
    print(data[0])
    max_diff = 0.0
    ...
    for i in range (0, len(data)):
        ...
        if np.abs(B['X'] - data[i]['X']) > 1e-3:
            print("\n\nthere is a mismatch in B['X'] - data[i]['X'] of size", B['X'] - data[i]['X'], B['X'] ,data[i]['X'])
